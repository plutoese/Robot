...
from libs.database.class_database import Database
import re
import pandas as pd
import numpy as np
...
class CgssDatabase(Database):
    ...
    # 返回某个变量的值
    def variable(self, variable=None, year=None, label=False,random_size=None):
        if year is None:
            year = self.year
    ...
    def variables(self, variables=None, year=None):
        ...
        for var in variables:
            sample[var[0]] = self.variable(variable=var[0], year=year, label=var[1])
...
if __name__ == '__main__':
    ...
    #print(cgdb.variables(variables=[['a2', True], ['a512', True]]))
    pdata = cgdb.variables(variables=[['a2', True], ['a512', True]])
