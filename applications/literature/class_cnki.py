...
if __name__ == '__main__':
    proxy_list = ['58.20.128.123:80', '36.7.151.29:8000',
                  '61.174.13.12:80', '112.90.179.153:4040',
                  '58.20.235.180:8000', '58.22.86.44:8000',
                  '101.226.249.237:80','111.1.89.254:80',
                  '112.16.87.24:80','111.1.89.254:80',
                  '61.163.91.75:3128']
    cnki_obj = Cnki(proxy_list[random.randint(0,len(proxy_list)-1)])
    #cnki_obj = Cnki(proxy='111.56.13.152:80')
    cnki_obj.set_query("SU='城镇化'*'工资'")
    cnki_obj.set_period(start_period="2010",end_period="2014")
    cnki_obj.set_subject(subjects=['经济与管理科学','社会科学Ⅱ辑'])
    cnki_obj.submit()
    cnki_obj.sort()
    ...
    cnki_obj.export_to_json()
    cnki_obj.close()

    P = json.load(open(r'E:\gitrobot\files\literature\literature_list.txt'))
    #F = open(r'E:\gitrobot\files\literature\literature_list.pkl', 'rb')
    #lite_list = pickle.load(F)
    print('finally, ',len(P))